...
# 数值清洗（去除逗号、空格、单位词，处理括号复数，返回数值类型）
...
def _infer_period(text: str, filename: str) -> Tuple[str, Optional[int]]:
    ...
    if "第一季度" in norm or "一季度" in norm:
        return "Q1", year
    if "半年度" in norm or "半年报" in norm:
        return "HY", year
    if "第三季度" in norm or "三季度" in norm:
        return "Q3", year
    if "年度报告" in norm or "年报" in norm:
        return "FY", year
...
SECTION_MARKERS = {
    ...
    "income_sheet": ["合并利润表"],      #, "利润表(合并)", "利润表（合并）", "利润表"
    "cash_flow": ["合并现金流量表"],      #, "现金流量表(合并)", "现金流量表（合并）", "现金流量表","现金流"
    "core_perf": ["主要财务指标", "(二)主要财务指标", "主要会计数据", "(一)主要会计数据", "主要会计数据和财务指标", "核心业绩指标", "(一)主要会计数据和财务指标"]
}

# 在PDF文本行中定位各报表章节的位置，返回章节的起止范围
...
ALIASES = {
    "core_performance_indicators_sheet": {
        ...
        "operating_revenue_yoy_growth": ["营业总收入同比增长", "营业总收入同比", "营业收入同比增长", "营业收入同比", "本期比上年同期增减", "营业收入"],
        "operating_revenue_qoq_growth": ["营业总收入季度环比增长", "营业收入季度环比增长", "环比增长"],
        "net_profit_10k_yuan": ["净利润", "归属于上市公司股东的净利润", "归属于母公司所有者的净利润", "归母净利润"],
        "net_profit_yoy_growth": ["净利润同比增长", "净利润同比", "归母净利润同比增长", "归母净利润同比", "归属于上市公司股东的净利润"],
        ...
        "net_profit_excl_non_recurring": ["扣除非经常性损益后的净利润", "扣非净利润", "归属于上市公司股东的扣除非经常性损益的净利润"],
        "net_profit_excl_non_recurring_yoy": ["扣非净利润同比增长", "扣非净利润同比", "归属于上市公司股东的扣除非经常性损益的净利润"],
    ...
    "balance_sheet": {
        "asset_cash_and_cash_equivalents": ["货币资金", "现金及现金等价物", "流动资产"],
        ...
        "liability_total_liabilities": ["负债合计"],   #, "总负债", "一年内到期的非流动负债", "负债总计"
        ...
        "equity_total_equity": ["所有者权益合计", "所有者权益（或股东权益）合计", "股东权益合计"]
    },
    "income_sheet": {
        "net_profit": ["归属于上市公司股东的净利润", "归属于母公司所有者的净利润", "归母净利润", "持续经营净利润", "归属于母公司股东的净利润"],
...
def classify_table(df: pd.DataFrame) -> Optional[str]:
    ...
    if any(k in text for k in ["合并资产负债表", "资产总计", "负债合计", "所有者权益合计", "货币资金"]):
        return "balance_sheet"
    if any(k in text for k in ["营业收入", "营业总收入", "营业利润", "利润总额", "净利润"]):
        return "income_sheet"
    if any(k in text for k in ["经营活动产生的现金流量净额", "投资活动产生的现金流量净额", "筹资活动产生的现金流量净额", "现金及现金等价物净增加额"]):
        return "cash_flow_sheet"
    if any(k in text for k in ["主要会计数据和财务指标", "基本每股收益", "净资产收益率", "主要财务指标", "主要会计数据"]):
        return "core_performance_indicators_sheet"
    return None

# 使用pdfplumber提取所有页面的表格
...
def detect_header_info(df: pd.DataFrame, table_type=None):
    ...
    for i in range(min(10, len(df))):
        ...
        for j, cell in enumerate(row):
            if any(k in cell for k in ["本报告期", "本期", "期末", "本年发生额", "本期发生额", "current", "current_date"]):
                header_info["current"] = j
                header_info["current_date"] = cell
                hit = True
            if any(k in cell for k in ["上年同期", "上期", "期初", "上期发生额", "上年末"]):
                header_info["previous"] = j
                hit = True
            if any(k in cell for k in ["同比", "同比增减", "比上年同期增减", "增减", "变动比例", "本期比上年同期增减", "本报告期比上年同期增减变动幅度", "本报告期比上年同期增减", "本期期末金额较上期期末变动比例"]):
                header_info["yoy"] = j
                hit = True
            if any(k in cell for k in ["环比", "季度环比", "比上季度增减", "本报告期末比上年度末增减变动幅度"]):
                header_info["qoq"] = j
                hit = True
        if hit:
    ...
    if header_info["current"] is None:
        ...
        if header_info["header_row_idx"] is not None:
            ...
            # 遍历表头行找最新日期列（本期）
            date_cols = []
            for j, cell in enumerate(header_row[2:]):
                # 匹配YYYY年MM月DD日格式
                if re.search(r"20\d{2}年\d{1,2}月\d{1,2}日", cell):
                    date_cols.append((j + 2, cell))
            if date_cols:
                # 按日期排序，取最新的为本期
                date_cols.sort(
...
def parse_core_performance(meta, table_items, lines, full_text, sec_range, income_data, balance_data, cash_data):
    ...
    data = {
        ...
        "roe": normal_field("roe", "yoy") or normal_field("roe"),
...
# ====================== 自动校验 ======================
# 核心校验函数
# 1.资产负债表平衡校验（总资产~负债+权益）
#2.利润表净利润~核心指标净利润
#3.三大活动之和~净增加额
#4合理性校验：EPS/ROE/资产负债率范围检查
...
if __name__ == "__main__":
    # 固定路径设置
    PDF_FOLDER = r"D:\竞赛\泰迪杯\B题数据及提交说明\附件2：财务报告\reports-上交所"
    OUTPUT_FOLDER = r"D:\竞赛\泰迪杯\上交所财务报告"  # 你也可以修改这个输出路径
