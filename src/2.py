...
"""
            ...
            for table_name in ["core_performance_indicators_sheet", "balance_sheet",
                ...
                if os.path.exists(csv_path):
                    ...
                    if "stock_abbr" in df.columns:
                        names = df["stock_abbr"].dropna().str.strip().unique().tolist()
    ...
    def _load_financial_examples(self) -> List[Tuple[str, str]]:
        ...
        return [
            ...
            ("金花股份近三年的利润总额变化趋势是什么",
             "SELECT report_year, total_profit FROM income_sheet WHERE stock_abbr = '金花股份' AND report_period = 'FY' AND report_year IN (2022, 2023, 2024) ORDER BY report_year"),

            ("金花股份近三年的经营活动现金流净额变化趋势是什么",
             "SELECT report_year, operating_cf_net_amount FROM cash_flow_sheet WHERE stock_abbr = '金花股份' AND report_period = 'FY' AND report_year IN (2022, 2023, 2024) ORDER BY report_year"),
        ]
    ...
    def clarify_intent_if_needed(self, query: str) -> Optional[str]:
        ...
        # 2. 更精准的公司名检测（支持“金花股份2022年...”这种写法）
        has_company = any(company in query.strip() for company in self.company_list)
        # 3. 指标检测
        indicator_keywords = ["收益", "利润", "收入", "资产负债率", "eps", "roe",
                              "净利润", "每股", "总资产", "总负债", "现金流",
        ...
        if has_indicator and not has_company:
            if any(x in query_lower for x in ["每股收益", "eps"]):
                ...
                return "请问你要查询哪一家公司的净利润？（例如：金花股份）"
    ...
    def build_prompt(self, query: str, retrieved_tables: List[str]) -> str:
        ...
        recent3 = f"({current_year - 2}, {current_year - 1}, {current_year})"
        recent2 = f"({current_year - 1}, {current_year})"
        p_base = (
            ...
            "4. 如果查询中没有具体的公司名称，且查询的是具体财务指标（如净利润、每股收益），请先生成追问，而不是随意选择一家公司。\n"
            "5. 严格使用 Schema 中真实存在的字段名。\n"
            "6. 【重要】禁止使用 CURDATE()、NOW()、YEAR() 等 MySQL 函数，SQLite 不支持！\n"
            "   '近三年' 必须写成：report_year IN {recent3}\n"
            "   '近两年' 必须写成：report_year IN {recent2}\n"
        )
...
"""}
    ...
    def query(self, natural_language: str) -> dict:
        """最终优化版 query：精准意图澄清 + 安全的上下文合并"""
        original_query = natural_language.strip()
        # ── Step 1：检查是否处于「待澄清」状态
        # 如果上一轮触发了意图澄清，_pending_clarify_query 保存了原始问题。
        # 本轮用户补充了公司名，则把两者合并后继续正常查询流程。
        if self._pending_clarify_query is not None:
            # 判断当前输入是否是公司名补充（包含已知公司名，且句子较短）
            is_company_supplement = (
                    any(company in original_query for company in self.company_list)
                    and len(original_query) <= 20
            )
            if is_company_supplement:
                # ★ 关键：用「澄清前原始问题 + 公司名」合并，而不是用上一条SQL
                merged_query = self._pending_clarify_query + " " + original_query
                log.info(
                    f"[多轮意图合并] 待澄清问题='{self._pending_clarify_query}' + 补充='{original_query}' → '{merged_query}'")
                self._pending_clarify_query = None  # 清除待澄清状态
                natural_language = merged_query
            else:
                # 用户输入的不像公司名（例如重新提问），放弃合并，重置状态
                log.info(f"[多轮意图] 补充内容不像公司名，重新作为独立查询处理")
                self._pending_clarify_query = None
                natural_language = original_query
        else:
            natural_language = original_query
        # Step 1: 意图澄清判断（使用原始输入）
        clarify_msg = self.clarify_intent_if_needed(original_query)
        log.info(
            f"[澄清判断] 查询: {original_query} | has_company: {any(c in original_query for c in self.company_list)} | clarify: {clarify_msg is not None}")
        # Step 2: 多轮合并 —— 只有当上一轮是澄清，且本次输入明显是补充时才合并
        merged = False
        if (self.history and len(self.history) >= 2 and
                "请问你要查询哪一" in str(self.history[-1][1]) and
                len(original_query) <= 30):  # 短输入才视为补充

            last_original = self.history[-2][0]
            # 安全合并：只在上一轮确实缺少公司时才合并
            if last_original and not self.clarify_intent_if_needed(last_original):
                natural_language = last_original + " " + original_query
                merged = True
                log.info(f"[多轮意图合并成功] {natural_language}")
            else:
                natural_language = original_query
        else:
            natural_language = original_query
        ...
        if clarify_msg:
            # ★ 保存「澄清前的原始问题」，等待下一轮用户补充公司名
            self._pending_clarify_query = natural_language
            self.history.append({
                "type": "clarify",
        ...
        sql = self.correct_sql_columns(sql)  # 去掉第二个参数

        result = self.execute_query(sql)

        # 保存历史（保存合并后的查询，方便下次判断）
        self.history.append((natural_language if merged else original_query, sql))
